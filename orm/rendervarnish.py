    ...
    if match_function == 'exist':
        ...
        if match_function == 'regex':
            ...
            if match_function == 'exact':
                ...
                regex = (reg_query_beg + esc_param + '='
                         + reg_query_wc + esc_value + reg_query_end)
            elif match_function == 'contains':
                regex = (reg_query_beg + esc_param + '='
                         + reg_query_wc + esc_value + reg_query_wc
                         + reg_query_end)
    if regex is None:
...
def make_sb_header_action(config_in, config_out, rule_id, indent_depth=0):
    return make_header_action(config_in, config_out, rule_id, southbound=True,
                              indent_depth=indent_depth)

def make_nb_header_action(config_in, config_out, rule_id, indent_depth=0):
    return make_header_action(config_in, config_out, rule_id, southbound=False,
                              indent_depth=indent_depth)
...
def make_redirect_action(config_in, config_out, rule_id, indent_depth=0):
    ...
    if scheme:
        config_out['sb'].append(
            indent(indent_depth)
            + 'variable.set("scheme", ' + vcl_safe_string(scheme) + ');'
        )
    else:
        config_out['sb'] += [
            indent(indent_depth) + 'if (std.port(server.ip) == 443) {',
            (indent(indent_depth + 1)
             + 'variable.set("scheme", "https");'),
            indent(indent_depth) + '} else {',
            (indent(indent_depth + 1)
             + 'variable.set("scheme", "http");'),
            indent(indent_depth) + '}'
        ]
    ...
    if domain:
        config_out['sb'].append(
            indent(indent_depth)
            + 'variable.set("domain", ' + vcl_safe_string(domain) + ');'
        )
    else:
        config_out['sb'].append(
            indent(indent_depth)
            + 'variable.set("domain", req.http.host);'
        )
    path = config_in.get('path', None)
    if path:
        make_path_mod_actions(path, config_out, rule_id,
                              indent_depth=indent_depth)
    config_out['sb'] += [
        indent(indent_depth)
...
def make_synth_resp_action(config_in, config_out, rule_id,
                           indent_depth=0):
    synth = (indent(3)
             + 'synthetic(' + vcl_safe_string(config_in) + ');')
    synth_clause = make_action_if_clause([synth],
                                         rule_id,
                                         indent_depth=2)
...
def make_action_if_clause(actions, rule_id, indent_depth=0):
    ...
    if_clause.append(indent(indent_depth)
                     + 'if (' +
                     make_vcl_match_variable_defined(rule_id)
                     + ') {')
    if_clause += actions
    if_clause.append(indent(indent_depth) + '}')
    return if_clause
...
def make_backend_action(config_in, config_out, rule_id, indent_depth=0):
    ...
    config_out['sb'] = [
        indent(indent_depth) + 'set req.http.X-ORM-ID = '
...
def make_trailing_slash_action(config_in, config_out, rule_id, indent_depth=0):
    ...
    if config_in == 'add':
        ...
        raise ORMInternalRenderException('ERROR: unhandled '
                                         + 'trailing slash action: '
                                         + config_in)
    action.append(indent(indent_depth)
                  + 'if (req.url ~ "' + regex + '") {')
    action.append(indent(indent_depth + 1)
                  + 'return (synth(307, '
...
def make_path_mod_actions(config_in, config_out, rule_id,
                          indent_depth=0):
    #pylint:disable=unused-argument
    actions = []
...
def make_path_prefix_action(prefix_config, indent_depth=0):
    ...
    if 'remove' in prefix_config:
        ...
        config.append(
            indent(indent_depth)
            + 'variable.set("path", '
            'regsub(variable.get("path"), {}, ""));'.format(
                vcl_safe_string(vcl_regex)
            )
        )
...
def make_path_replace_action(replace_config, indent_depth=0):
    ...
    if 'from_regex' in replace_config:
        vcl_regex = make_vcl_path_regex(replace_config['from_regex'],
                                        'regex',
                                        ignore_case)
        vcl_sub = replace_config.get('to_regsub', vcl_sub)
    elif 'from_exact' in replace_config:
        vcl_regex = make_vcl_path_regex(replace_config['from_exact'],
                                        'exact',
                                        ignore_case)
...
class RenderVarnish(RenderOutput):
    ...
    def handle_condition_list(self, operator, match_tree_list, negate=False,
                              indent_depth=0):
        first = True
        expr = '!' if negate else ' '
        if len(match_tree_list) == 1:
            return expr + self.parse_match_tree(match_tree_list[0],
                                                indent_depth)
    ...
    def parse_match_tree(self, match_tree, indent_depth=0, negate=False):
        ...
        if 'not' in match_tree:
            return self.parse_match_tree(match_tree['not'],
                                         indent_depth=indent_depth,
                                         negate=True)
        raise ORMInternalRenderException('ERROR: unhandled condition operator: '
                                         + str(match_tree.keys))
    ...
    def make_condition(self,
        ...
        condition.append(indent(indent_depth + 1)
    ...
    def make_match_sub(self, match_tree, rule_id, match_sub_name):
        ...
        sub += self.make_condition(match_tree,
                                   rule_id,
                                   indent_depth=indent_depth + 1)
        sub.append('}')
        return sub

    def make_actions(self, action_config, rule_id, domain=None,
                     match_sub_name=None, indent_depth=0, is_global=False):
        ...
        supported_actions = [
            {'name': 'https_redirection',
             'func': make_https_redir_action}, # Must be first
            {'name': 'trailing_slash',
             'func': make_trailing_slash_action}, # Must be second
        ...
        if is_global:
            ...
            if sb:
                sb.insert(0, indent(indent_depth + 1) +
    ...
    def make_southbound_actions(self, domains):
        ...
        for domain in domains:
            ...
            config.append(
                indent(1) +
                'if (req.http.host == ' + vcl_safe_string(domain) + ') {'
            )
            for line in actions:
            ...
            if default_actions:
                config.append(
                    indent(2) +
                    '# ORM: Default southbound actions for ' + domain
                )
    ...
    def make_northbound_actions(self, domains):
        ...
        for domain in domains:
            ...
            if default_actions:
                config.append(
                    indent(1) +
                    '# ORM: Default northbound actions for ' + domain
                )
