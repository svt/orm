    ...
    for match_function in match_functions:
        if match_function in paths_config:
            for path in paths_config[match_function]:
                ...
                expr_list.append(create_match_tree_expr('path',
                                                        match_function,
                                                        inp))
...
def parse_match_query(query_config):
    expr_list = []
    match_functions = ['exist', 'exact', 'regex', 'begins_with', 'ends_with',
                       'contains']
    ...
    for match_function in match_functions:
        if match_function in query_config:
            if match_function == 'exist':
                inp = {
                    'parameter': query_config['parameter']
                }
                if ignore_case:
                    inp['ignore_case'] = ignore_case
                expr_list.append(create_match_tree_expr('query',
                                                        match_function,
                                                        inp))
            else:
...
def parse_match_binary_operator(operator, expressions):
    ...
    if operator == 'any':
        ...
        raise ORMInternalParserException('ERROR: unhandled binary operator: '
                                         +  operator)
    expr_list = []
    for expr in expressions:
        if 'paths' in expr:
            ...
            raise ORMInternalParserException('ERROR: unhandled key in: '
                                             + str(expr.keys()))
    return {op: expr_list}

def parse_match_domains(domains):
    matches = list(map(lambda domain: {
        'match': {
            'source': 'domain',
            'function': 'exact',
            'input': domain
...
def traverse_match_tree(func, match_tree, negate=False):
    ...
    if 'and' in match_tree:
        data_out = traverse_condition_list(func,
                                           match_tree['and'],
                                           'and',
                                           negate=negate)
    elif 'or' in match_tree:
        ...
        data_out = traverse_match(func,
                                  match_tree['match'],
                                  negate=negate)
    elif 'not' in match_tree:
        data_out = traverse_match_tree(func,
                                       match_tree['not'],
                                       negate=(not negate))
    else:
        raise ORMInternalParserException('ERROR: unhandled condition operator: '
                                         + str(match_tree.keys))
    return data_out
...
def parse_document(doc):
    ...
    if doc.get('schema_version', 1) == 1:
        ...
        raise ORMInternalParserException('schema_version %s not supported'
                                         % doc['schema_version'])
...
def normalize(string):
    return re.sub(r"(_)\1{1,}",
                  r'\1',
                  re.sub(r'[^a-z0-9]', '_', string)).strip('_')
...
def set_rule_defaults(rule, defaults):
    ...
    if defaults.get('https_redirection', False):
        if 'redirect' not in rule['actions']:
            rule['actions']['https_redirection'] = (
                rule['actions'].get('https_redirection', True)
            )
        ...
        for doc in yml_docs:
            ...
            for domain, rules in parsed_doc['rules'].items():
                ...
                for rule in rules:
                    ...
                    rule_copy['_rule_id'] = get_unique_id(name_counter,
                                                          rule['description'])
